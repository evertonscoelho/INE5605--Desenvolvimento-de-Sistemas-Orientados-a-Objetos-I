...
public class TabelaMostrarDadosVeiculos extends JPanel implements
		ActionListener {
	...
	public TabelaMostrarDadosVeiculos(Deposito deposito) {
	...
	private void define() {
		...
		lde = new JLabel("de ");
		try {
			String total = "" + deposito.getQntdVeiculosCadastrados();
		...
		colunas[0] = "Placa";
		colunas[1] = "Ano de Fabricação";
		colunas[2] = "CPF do Proprietário";
	...
	private void insertLinhaTabela() {
		try {
			if (tfbusca.getText().trim().length() > 0) {
				...
				try {
					Veiculo[] veiculos = deposito.getVeiculosEmArray(
							numeroLinhas, inicial);
					for (int x = 0; x <= veiculos.length - 1; x++) {
						if (veiculos[x] != null) {

							CPF = "" + veiculos[x].getCPFCondutor();
							anoFabricacao = "" + veiculos[x].getAnoFabricacao();
							valores[0] = veiculos[x].getPlaca();
							valores[1] = anoFabricacao;
							valores[2] = CPF;
